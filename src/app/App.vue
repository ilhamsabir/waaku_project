	...
	<div class="min-h-screen bg-gradient-to-br from-green-50 via-blue-50 to-purple-50">
		...
		<div class="max-w-7xl mx-auto px-6 py-8 space-y-8">
			...
			<div class="flex justify-end mb-6">
				<button @click="openModal" class="px-6 py-3 bg-gradient-to-r from-green-500 to-blue-500 hover:from-green-600 hover:to-blue-600 text-white font-semibold rounded-xl transition-all duration-200 transform hover:scale-105 shadow-lg hover:shadow-xl flex items-center space-x-3">
			...
			<div class="space-y-6">
				...
				<div v-if="sessions.length === 0" class="text-center py-16">
					...
					<button @click="openModal" class="inline-flex items-center px-6 py-3 bg-gradient-to-r from-green-500 to-blue-500 hover:from-green-600 hover:to-blue-600 text-white font-semibold rounded-xl transition-all duration-200 transform hover:scale-105 shadow-lg hover:shadow-xl space-x-2">
						<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
							<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"/>
						</svg>
					...
					<div v-if="s.error" class="p-6 bg-red-50">
						<div class="flex items-center space-x-3">
							...
							<div class="flex-1">
								...
								<button @click="restartSession(s.id)" class="mt-3 px-4 py-2 bg-red-600 hover:bg-red-700 text-white font-medium rounded-lg transition-colors">
									🔄 Try Restart
								</button>
							</div>
						</div>
...
function initSockets() {
	// Avoid duplicate listeners during hot reload
	if (detachSockets) {
		detachSockets()
		detachSockets = null
	}
